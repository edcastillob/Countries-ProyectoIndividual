...
import {
  ...
  orderPopulation,
  showActivities,
  getActivitiesCountry,
  pagination,
  orderAscDesCountriesName,
...
export const Countries = () => {
  ...
  useEffect(() => {
    if (countryRegion.length === 5 || countryRegion.length > 10) {
      ...
      setOptionCountry(countriesPagination);
    }
  }, [
    showActivitiesCountry,
    countriesState,
    countryRegion,
    countryPopulation,
    countriesName,
  ...
  return (
    <div>
      ...
      <select
        ...
        name="activities"
        id="activities"
        onChange={handleActivitiesCountry}
      >
      ...
      {activitiesCountry?.map((activTurist) => {
        const countryNames = activTurist.searchActivityCountry.map(
          (country) => {
            let result = countriesPopulation?.find(
              (element) => element.id === country.CountryId
            );
            return result;
          }
        );
        ...
        return (
          <Activity
            ...
            countryID={countryNames.map((country) => (
